...
const { startV2TurnkeyStudio } = require('../src/venue/v2/turnkey-studio');

function usage() {
  process.stderr.write(
    'Usage: npm run venue:studio:v2 -- <workspace-directory> [--port <0-65535>]\n',
  );
}
...
async function main(argv = process.argv.slice(2)) {
  ...
  try {
    const runtime = await startV2TurnkeyStudio({
      workspaceDirectory: args.workspaceDirectory,
      port: args.port,
    });
    process.stdout.write([
      'HiVenues Flagship v2 Venue Studio',
      `Workspace: ${path.resolve(args.workspaceDirectory)}`,
      `Open: ${runtime.url}`,
      'Loopback-only: no production routing, publishing, deployment, Hive writes/signing, payments, or infrastructure effects.',
