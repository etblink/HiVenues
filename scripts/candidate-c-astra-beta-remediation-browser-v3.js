...
function observePage(page, evidence) {
  page.on('console', (message) => {
    ...
    if (expectedResource) { evidence.adversarialResourceConsoleErrors.push({ status: Number(expectedResource[1]), text }); return; }
...
async function fillCreatorStageOne(page, values) {
...
async function fillCreatorStageTwo(page, values) {
  await page.locator(`[name="presenceMode"][value="${values.presenceMode}"]`).check();
  await page.locator('[name="presenceLabel"]').fill(values.presenceLabel);
  if (values.address !== undefined) await page.locator('[name="address"]').fill(values.address);
...
async function fillCreatorStageThree(page, direction) {
  await page.locator(`[name="direction"][value="${direction}"]`).check();
...
async function createSecondary(page, origin) {
  ...
  await fillCreatorStageOne(page, {
    displayName: 'Astra Secondary Lab', archetype: 'synthetic online host',
    tagline: 'A second synthetic workspace for restart qualification.',
    purpose: 'Prove two independently durable host workspaces.',
  });
  await fillCreatorStageTwo(page, {
    presenceMode: 'online', presenceLabel: 'Online-only synthetic qualification space', address: '',
    contact: 'secondary qualification contact', timezone: 'America/Los_Angeles',
  ...
  await fillCreatorStageThree(page, 'editorial');
  await page.locator('[name="participation"]').fill('Read the synthetic update.');
  await Promise.all([
    page.waitForURL((url) => url.pathname === `/candidate-c/studio/${SECONDARY_SLUG}`),
    page.getByRole('button', { name: 'Create my first draft' }).click(),
  ]);
...
async function main() {
  ...
  const evidence = {
    candidate: EXACT_SHA, tree: EXACT_TREE,
  ...
  observeContext(context, evidence); observeContext(mobileContext, evidence);
  let page = await context.newPage();
  const mobile = await mobileContext.newPage();
  for (const current of [page, mobile]) { current.setDefaultTimeout(15000); current.setDefaultNavigationTimeout(25000); observePage(current, evidence); }
  ...
  try {
    ...
    await page.goto(`${origin}/candidate-c/new`, { waitUntil: 'networkidle' });
    await fillCreatorStageOne(page, {
      displayName: 'Astra Remediation Lab', archetype: 'synthetic community venue',
      tagline: 'A synthetic place for product regression.', purpose: 'Exercise the complete remediation without real customer facts.',
    });
    await fillCreatorStageTwo(page, {
      presenceMode: 'physical', presenceLabel: 'Synthetic Test City · evenings', address: '100 Fictional Way, Test City, NV 00000',
      contact: '(702) 555-0147', timezone: 'Not/A_Real_Zone',
    ...
    await fillCreatorStageThree(page, 'poster');
    await page.locator('[name="participation"]').fill(INVITATION);
    await page.getByText('Optional · Add a first Activity with real facts').click();
    await page.locator('[name="activityTitle"]').fill(ACTIVITY);
    await page.locator('[name="activityDescription"]').fill('A synthetic scheduled activity for reschedule, Release and lifecycle qualification.');
    await page.locator('[name="activityStartsLocal"]').fill('2026-10-30T18:00');
    await page.locator('[name="activityEndsLocal"]').fill('2026-10-30T17:00');
    ...
    assert.equal(rescheduled.id, originalActivity.id); assert.equal(rescheduled.slug, originalActivity.slug); assert.equal(rescheduled.mediaId, originalActivity.mediaId);
    assert.notEqual(rescheduled.startsAt, liveBeforeSchedule); assert.equal(store.publicSnapshot(SLUG).draft.activities[0].startsAt, liveBeforeSchedule);
    ...
    assert.match(scheduledIcsText, /STATUS:CONFIRMED/); assert.match(scheduledIcsText, /SUMMARY:Synthetic Evening Lab/);
    ...
    await applyDirection(page, origin, SLUG, 'Editorial field notes'); await publishCurrent(page, origin, SLUG);
    await capturePublicFamily(page, mobile, origin, SLUG, 'editorial', evidence);
    await applyDirection(page, origin, SLUG, 'Hospitality table'); await publishCurrent(page, origin, SLUG);
    ...
    const contactCases = [['operator@example.test', 'email'], ['(702) 555-0199', 'phone'], ['https://astra-remediation.example.test/', 'web'], ['Ask at the front desk', 'text']];
    for (const [value] of contactCases) { await quickContact(page, origin, SLUG, value); assert.equal(store.snapshot(SLUG).draft.facts.contact, value); }
    for (const [value] of contactCases) { await deepContact(page, origin, SLUG, value); assert.equal(store.snapshot(SLUG).draft.facts.contact, value); }
    for (const [value, kind] of contactCases) {
      await deepContact(page, origin, SLUG, value); await publishCurrent(page, origin, SLUG); await page.goto(`${origin}/candidate-c/${SLUG}`, { waitUntil: 'networkidle' });
      ...
      if (kind === 'text') { assert.match(await page.locator('body').textContent(), /Ask at the front desk/); assert.equal(await page.getByRole('link', { name: value, exact: true }).count(), 0); }
    }

    await setActivityStatus(page, origin, SLUG, ACTIVITY, 'completed', 'Synthetic completed-state proof.'); await publishCurrent(page, origin, SLUG);
    await page.goto(`${origin}/candidate-c/${SLUG}`, { waitUntil: 'networkidle' });
    const completedBody = await page.locator('body').textContent(); assert.match(completedBody, /PAST/); assert.match(completedBody, /Already happened/); assert.doesNotMatch(completedBody, /\bNEXT\b/);
    ...
    await page.getByRole('button', { name: 'Save headline' }).click(); assert.equal((await headlineResponse).status(), 200);
    await page.goto(`${origin}/candidate-c/studio/${SLUG}/release`, { waitUntil: 'networkidle' });
    assert.match(await page.locator('body').textContent(), /Release impact/); assert.match(await page.locator('body').textContent(), /Headline changed/);
    ...
    await page.goto(`${origin}/candidate-c/studio/${SLUG}/release`, { waitUntil: 'networkidle' });
    const restoreLinks = page.getByRole('link', { name: 'Review restore', exact: true }); assert((await restoreLinks.count()) >= 2);
    await restoreLinks.nth(1).click(); await page.waitForURL((url) => /\/releases\/[^/]+\/restore$/.test(url.pathname));
    assert.match(await page.locator('body').textContent(), /does not roll the live website back/i);
    await page.getByRole('button', { name: 'Use this as working version' }).click(); await page.waitForURL((url) => url.pathname === `/candidate-c/studio/${SLUG}`);
    assert.equal(store.publicSnapshot(SLUG).draftDigest, liveBeforeRestore);

    const staleRelease = await context.newPage(); observePage(staleRelease, evidence); staleRelease.setDefaultTimeout(15000);
    await staleRelease.goto(`${origin}/candidate-c/studio/${SLUG}/release`, { waitUntil: 'networkidle' });
    await page.goto(`${origin}/candidate-c/studio/${SLUG}`, { waitUntil: 'networkidle' }); await openStudioButton(page, 'Page', 'First impression');
    ...
    await page.getByRole('button', { name: 'Save headline' }).click(); assert.equal((await newerResponse).status(), 200);
    ...
    await staleRelease.getByRole('button', { name: 'Publish website release' }).click(); assert.equal((await staleReleaseResponse).status(), 409);
    assert.match(await staleRelease.locator('body').textContent(), /newer version exists/i); await staleRelease.close();

    await page.goto(`${origin}/candidate-c/studio/${SLUG}/media-library`, { waitUntil: 'networkidle' });
    await page.locator('#cc-hero-file').setInputFiles(validPath); await page.locator('#cc-hero-alt').fill('Synthetic blue qualification pixel'); await page.locator('#cc-hero-caption').fill('Synthetic local-only qualification media');
    await Promise.all([page.waitForURL((url) => url.searchParams.get('imported') === 'hero'), page.getByRole('button', { name: /Replace placeholder with real image|Replace primary image/ }).click()]);
    await page.locator('#cc-logo-file').setInputFiles(validPath); await page.locator('#cc-logo-alt').fill('Synthetic qualification logo'); await page.locator('#cc-logo-caption').fill('Synthetic local-only qualification mark');
    await Promise.all([page.waitForURL((url) => url.searchParams.get('imported') === 'logo'), page.getByRole('button', { name: /Import official logo|Replace official logo/ }).click()]);
    ...
    await page.getByRole('button', { name: 'Save focal point' }).click(); assert.equal((await focalResponse).status(), 200);
    ...
    assert(importedHero.asset && importedLogo.asset); assert.deepEqual(importedHero.focal, { x: 73, y: 31 });
    await capture(page, '12-studio-desktop-real-media.png', 'studio-desktop-real-media', evidence);
    await publishCurrent(page, origin, SLUG);
    await page.goto(`${origin}/candidate-c/${SLUG}`, { waitUntil: 'networkidle' }); await capture(page, '13-real-media-public-desktop.png', 'real-media-public-desktop', evidence);
    await mobile.goto(`${origin}/candidate-c/${SLUG}`, { waitUntil: 'networkidle' }); await capture(mobile, '14-real-media-public-390.png', 'real-media-public-390', evidence);

    await page.goto(`${origin}/candidate-c/studio/${SLUG}/media-library`, { waitUntil: 'networkidle' });
    const beforeRejectedMedia = store.snapshot(SLUG);
    await page.locator('#cc-hero-file').setInputFiles(oversizedPath); await page.locator('#cc-hero-alt').fill('Synthetic oversized valid PNG');
    ...
    await page.getByRole('button', { name: /Replace placeholder with real image|Replace primary image/ }).click(); assert.equal((await oversizedResponse).status(), 413);
    assert.match(await page.locator('body').textContent(), /larger than the 8 MiB limit/i); assert.equal(store.snapshot(SLUG).revision, beforeRejectedMedia.revision);
    await page.getByRole('link', { name: 'Choose another image', exact: true }).click();
    await page.locator('#cc-hero-file').setInputFiles(malformedPath); await page.locator('#cc-hero-alt').fill('Synthetic invalid media');
    ...
    await page.getByRole('button', { name: /Replace placeholder with real image|Replace primary image/ }).click(); assert.equal((await malformedResponse).status(), 400);
    assert.match(await page.locator('body').textContent(), /Use a JPEG or PNG image/i);

    await applyDirection(page, origin, SLUG, 'Hospitality table'); await setActivityStatus(page, origin, SLUG, ACTIVITY, 'scheduled', ''); await publishCurrent(page, origin, SLUG);
    ...
    await page.locator('.cc-rsvp button[type="submit"]').click(); assert.equal((await rsvpResponse).status(), 200); assert.equal(store.diagnostics().local.rsvps, 1);

    await page.goto(`${origin}/candidate-c/studio/${SLUG}/urgent?activity=${originalActivity.id}`, { waitUntil: 'networkidle' });
    await page.locator('input[name="lifecycle"][value="cancelled"]').check(); await page.locator('#cc-urgent-note').fill('STALE urgent cancellation must not publish.');
    await page.getByRole('button', { name: 'Review the change' }).click(); await page.waitForURL((url) => url.pathname.startsWith(`/candidate-c/studio/${SLUG}/urgent/`));
    const staleUrgent = page;
    const advance = await context.newPage(); advance.setDefaultTimeout(15000); observePage(advance, evidence);
    await advance.goto(`${origin}/candidate-c/studio/${SLUG}`, { waitUntil: 'networkidle' }); await openStudioButton(advance, 'Page', 'First impression');
    ...
    await advance.getByRole('button', { name: 'Save headline' }).click(); assert.equal((await advanceSave).status(), 200); await publishCurrent(advance, origin, SLUG);
    ...
    await staleUrgent.getByRole('button', { name: 'Publish urgent update' }).click(); assert.equal((await staleUrgentResponse).status(), 409);
    assert.equal(store.publicSnapshot(SLUG).liveReleaseId, liveAfterAdvance); await staleUrgent.close(); page = advance;
    ...
    await page.getByRole('button', { name: 'Save headline' }).click(); assert.equal((await unpublishedSave).status(), 200);
    await page.goto(`${origin}/candidate-c/studio/${SLUG}/urgent?activity=${originalActivity.id}`, { waitUntil: 'networkidle' });
    await page.locator('input[name="lifecycle"][value="cancelled"]').check(); await page.locator('#cc-urgent-note').fill('Synthetic cancellation is live; unrelated Studio work stays draft-only.');
    await page.getByRole('button', { name: 'Review the change' }).click(); await page.waitForURL((url) => url.pathname.startsWith(`/candidate-c/studio/${SLUG}/urgent/`));
    ...
    await page.getByRole('button', { name: 'Publish urgent update' }).click(); assert.equal((await urgentResponse).status(), 303);
    await page.waitForURL((url) => url.pathname === `/candidate-c/studio/${SLUG}` && url.searchParams.get('urgent') === '1');
    const afterUrgent = store.publicSnapshot(SLUG); const workingAfterUrgent = store.snapshot(SLUG);
    assert.equal(afterUrgent.draft.activities.find((item) => item.id === originalActivity.id).lifecycle, 'cancelled');
    assert.equal(workingAfterUrgent.draft.facts.tagline, UNPUBLISHED); assert.notEqual(afterUrgent.draft.facts.tagline, UNPUBLISHED);
    const urgentRelease = workingAfterUrgent.releases.find((item) => item.id === afterUrgent.liveReleaseId); assert.equal(urgentRelease.kind, 'urgent');
    await page.goto(`${origin}/candidate-c/${SLUG}/activities/${originalActivity.slug}`, { waitUntil: 'networkidle' });
    ...
    const cancelledIcsText = await cancelledIcs.text(); assert.match(cancelledIcsText, /STATUS:CANCELLED/); assert.match(cancelledIcsText, /SUMMARY:Cancelled: Synthetic Evening Lab/);
    ...
    const secondaryBeforeRestart = store.snapshot(SECONDARY_SLUG); const primaryBeforeRestart = store.snapshot(SLUG); const primaryLiveBeforeRestart = store.publicSnapshot(SLUG);
    const heroBeforeRestart = primaryBeforeRestart.draft.media.find((item) => item.id !== `media-${SLUG}-logo`); const logoBeforeRestart = primaryBeforeRestart.draft.media.find((item) => item.id === `media-${SLUG}-logo`);
    assert.equal(store.workspace(SLUG).rsvps.size, 1); assert(heroBeforeRestart.asset && logoBeforeRestart.asset);

    await page.close(); await stopServer(server);
    ...
    page = await context.newPage(); page.setDefaultTimeout(15000); page.setDefaultNavigationTimeout(25000); observePage(page, evidence);
    const primaryAfterRestart = store.snapshot(SLUG); const primaryLiveAfterRestart = store.publicSnapshot(SLUG); const secondaryAfterRestart = store.snapshot(SECONDARY_SLUG);
    assert.equal(primaryAfterRestart.liveReleaseId, primaryBeforeRestart.liveReleaseId); assert.equal(primaryLiveAfterRestart.liveReleaseId, primaryLiveBeforeRestart.liveReleaseId);
    assert.equal(primaryAfterRestart.draft.facts.tagline, UNPUBLISHED); assert.notEqual(primaryLiveAfterRestart.draft.facts.tagline, UNPUBLISHED);
    assert.deepEqual(primaryAfterRestart.draft.media.find((item) => item.id !== `media-${SLUG}-logo`).focal, { x: 73, y: 31 });
    assert.equal(store.workspace(SLUG).rsvps.size, 1); assert.equal(secondaryAfterRestart.draft.facts.tagline, 'SECONDARY-WORKSPACE-DURABLE');
    assert.equal(secondaryAfterRestart.liveReleaseId, secondaryBeforeRestart.liveReleaseId);

    await page.goto(`${origin}/candidate-c/studio/${SLUG}`, { waitUntil: 'networkidle' }); await capture(page, '15-restart-studio-desktop.png', 'restart-studio-desktop', evidence);
    await mobile.goto(`${origin}/candidate-c/studio/${SLUG}`, { waitUntil: 'networkidle' }); await capture(mobile, '16-studio-390-normal.png', 'studio-390-normal', evidence);
    await openStudioButton(mobile, 'Page', 'First impression'); await capture(mobile, '17-studio-390-contextual.png', 'studio-390-contextual', evidence);
    await mobile.goto(`${origin}/candidate-c/${SLUG}`, { waitUntil: 'networkidle' }); await capture(mobile, '18-native-390-public-final.png', 'native-390-public-final', evidence);
    ...
    evidence.proof = {
      ...
      reschedule: { activityIdPreserved: primaryAfterRestart.draft.activities.find((item) => item.id === originalActivity.id).id === originalActivity.id, releasedStartsAt: rescheduled.startsAt, scheduledIcsCaptured: true, cancelledIcsCaptured: true },
      parity: { families: ['poster', 'editorial', 'hospitality'], invitation: INVITATION, category: OFFER_CATEGORY, detail: OFFER_DETAIL },
      safeOversizedValidPng: true, safeMalformedMedia: true,
      contactClassesThroughQuickAndDeep: contactCases.map(([value, kind]) => ({ value, kind })),
      quickDeepLookBarGold: true, completedAndCancelledLifecycle: true,
      restoreDidNotChangeLive: true, staleReleaseRejected: true, staleUrgentRejected: true, urgentPreservedUnpublishedDraft: true,
      expectedAdversarialResourceStatuses: expectedAdversarialStatuses,
      restart: { persisted: true, multiHost: true, hosts: [SLUG, SECONDARY_SLUG], releases: primaryAfterRestart.releases.length, liveReleaseId: primaryAfterRestart.liveReleaseId, heroSha256: heroBeforeRestart.asset.sha256, logoSha256: logoBeforeRestart.asset.sha256, focal: primaryAfterRestart.draft.media.find((item) => item.id !== `media-${SLUG}-logo`).focal, rsvps: store.workspace(SLUG).rsvps.size, urgentReleaseId: urgentRelease.id, restoredWorkingStatePreservedThroughLaterWork: true },
      native390PublicAndStudio: true, external,
    };
    evidence.summary = {
    ...
    await mobileContext.close(); await browser.close(); await stopServer(server);
  }
}
