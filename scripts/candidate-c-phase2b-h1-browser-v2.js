...
function readClientInventory() {
  ...
  const files = CLIENT_FILES.map((file) => {
    ...
    return { file, lines, bytes: Buffer.byteLength(source), usesBrowserStorage: /localStorage|sessionStorage|indexedDB/.test(source), usesFetch: /fetch\(/.test(source) };
  });
  ...
  walk(viewRoot);
  return { files, totalFiles: files.length, totalLines: files.reduce((sum, item) => sum + item.lines, 0), inlineScriptsInViews: inlineScripts, libraries: ['htmx.org (local, unmodified)'] };
}
...
async function main() {
  ...
  async function openButton(page, menu, name) {
    await page.locator('.cc-studio-commandbar summary').filter({ hasText: menu }).click();
  ...
  async function measureEdit(page, label, routeSuffix, act) {
    ...
    const record = {
      label, route: routeSuffix, status: response.status(), targetedRegions: targeted, oobRegions: oob, totalRegions: targeted + oob,
      responseBytes: Buffer.byteLength(html), revisionBefore: before, revisionAfter: before + 1, serverRevisionAfter: store.snapshot(SLUG).revision,
      focus, selection, saveState: await page.locator('#cc-save-state').textContent(),
    };
  ...
  try {
    ...
    assert.equal(await page.locator('.cc-studio-rail').count(), 0);

    await openButton(page, 'Page', 'First impression');
    ...
    await openButton(page, 'Activities', 'Sunday Table — Harvest Supper');
    ...
    await measureEdit(page, 'Activity · status (E draft path)', 'activity-status', async () => {
      ...
      const note = page.locator('#cc-activity-status-note, #cc-activity-note').first();
      await note.fill('Measured cancellation note.');
      await page.getByRole('button', { name: 'Save status' }).click();
    });
    assert.equal(await page.locator('#candidate-canvas [data-activity-status="cancelled"]').count(), 1);

    await page.keyboard.press('Escape');
    await openButton(page, 'Offers', 'Coal-roasted carrots');
    ...
    await page.keyboard.press('Escape');
    await openButton(page, 'Site', 'Look');
    ...
    await page.keyboard.press('Escape');
    await openButton(page, 'Site', 'Voice');
    ...
    await page.keyboard.press('Escape');
    await openButton(page, 'Site', 'Participation');
    await measureEdit(page, 'Participation · contact (D)', 'connect', async () => {
      await page.locator('#cc-contact').fill('measured@harborandhearth.example');
      await page.getByRole('button', { name: 'Save contact' }).click();
    });

    await page.keyboard.press('Escape');
    await openButton(page, 'Site', 'Hero media');
    ...
    await page.keyboard.press('Escape');
    await openButton(page, 'Page', 'Page rhythm');
    ...
    assert.equal(Number(conflict.renderedRevision), conflict.serverRevision - 1);
    ...
    assert.equal(summary.customClientFiles, 1);
    assert(summary.customClientLines < 150, `client island grew to ${summary.customClientLines} lines`);
    assert.equal(summary.maxRegionsPerEdit, 3);
    ...
    fs.writeFileSync(path.join(OUTPUT_ROOT, 'manifest.json'), `${JSON.stringify({
      ...
      edits, conflict, afterReload, history, storage, urgentScripts,
      noJavaScript: { urgentPublished: true, setupSaved: true },
      screenshots, externalRequests, consoleErrors, expectedConflictDiagnostics, diagnostics, summary,
      finding: 'H1_CONTINUE',
    }, null, 2)}\n`);
  } finally {
    await browser.close();
