...
async function main() {
  ...
  const evidence = {
    ...
    fixture: {
      synthetic: true,
      purpose: 'Issue #265 browser-only add-later Activity → Release → urgent status qualification.',
      slug: SLUG,
    },
    screenshots: [],
    audits: [],
  ...
  function observe(targetPage) {
    ...
    targetPage.on('pageerror', (error) => evidence.pageErrors.push(error.message));
  }
  observe(page);
  ...
  try {
    ...
    const headlineResponse = page.waitForResponse((response) => (
      response.url().endsWith(`/candidate-c/studio/${SLUG}/tagline`) && response.request().method() === 'POST'
    ));
    ...
    const urgentResponse = page.waitForResponse((response) => (
      response.url().includes(`/candidate-c/studio/${SLUG}/urgent/${operationId}/publish`) && response.request().method() === 'POST'
    ));
