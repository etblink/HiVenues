...
const { createCandidateCH1Spike } = require('../test/support/candidate-c-h1-spike');
...
async function main() {
  ...
  try {
    ...
    await tabB.waitForTimeout(150);
    assert.equal(await tabB.locator('#conflict').count(), 1, 'HTMX must surface the server 409 conflict in the Studio UI');
