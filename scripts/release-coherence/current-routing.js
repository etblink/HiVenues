...
const NEXT_SUCCESSOR_OPERATION = 'TEST_SUITE_LIVE_INVARIANT_AND_CROSS_PLATFORM_SCOPE_AUDIT';
...
function assertCurrentRoutingBlock(relativePath) {
  ...
  const required = [
    ...
    [/^HIVE_IDENTITY_KEY_MANAGEMENT_AUDIT = ACCEPTED__DESIGN_ONLY$/m, 'Hive identity/key-management audit must remain accepted as design-only'],
    [/^THREADS_AUTOMATION_IDENTITY = RETAIN__POSTING_ONLY_DEFAULT$/m, 'dedicated Threads automation identity decision must remain preserved'],
    [/^BENEFICIARY_POLICY_DESIGN = ACCEPTED__IMPLEMENTATION_NOT_STARTED$/m, 'beneficiary design must remain accepted without implying activation'],
    [new RegExp(`^NEXT_OPERATION = ${NEXT_SUCCESSOR_OPERATION}$`, 'm'), 'next operation must be the deterministic test/Windows scope audit'],
...
function assertLivingRoutingCoherence({ readme, docsReadme, roadmap }) {
  ...
  requireMatch(readme, /production transition is withheld/i, 'README must preserve the HV-8 stop decision');
  requireMatch(readme, /community pulse is accepted/i, 'README must record the accepted homepage slice');
  requireMatch(readme, /Recent activity.*accepted/i, 'README must record the accepted return-loop slice');
  ...
  requireMatch(readme, /identity.*key-management audit.*accepted/is, 'README must record the accepted identity/key-management design'],
  requireMatch(readme, /dedicated Threads.*Posting/i, 'README must preserve the selected Threads automation model');
  requireMatch(readme, /venue beneficiary/i, 'README must record venue beneficiary design');
  requireMatch(readme, /creator donation/i, 'README must record voluntary creator-donation design');
  requireMatch(readme, /test-suite.*Windows.*scope audit/is, 'README must identify the selected engineering-health audit');
  ...
  requireMatch(docsReadme, /HIVE_IDENTITY_KEY_MANAGEMENT_AUDIT = ACCEPTED__DESIGN_ONLY/, 'docs index must record identity audit acceptance');
  requireMatch(docsReadme, /BENEFICIARY_POLICY_DESIGN = ACCEPTED__IMPLEMENTATION_NOT_STARTED/, 'docs index must record beneficiary design without activation');
  requireMatch(docsReadme, /TEST_SUITE_LIVE_INVARIANT_AND_CROSS_PLATFORM_SCOPE_AUDIT/, 'docs index must route to the test-scope audit');
  requireMatch(docsReadme, /production transition.*withheld/i, 'docs index must preserve HV-8 stop decision');
  ...
  requireMatch(roadmap, /Local operator launcher.*PR #104/is, 'roadmap must record accepted local launcher');
  requireMatch(roadmap, /Hive identity.*beneficiary.*adjudication/is, 'roadmap must record completed identity/beneficiary adjudication');
  requireMatch(roadmap, /Current operation.*test-suite.*Windows.*scope/is, 'roadmap must select the test/Windows scope audit');
