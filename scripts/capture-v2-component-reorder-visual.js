...
const OUTPUT = path.resolve(
  ROOT,
  process.env.V2_REORDER_REVIEW_ROOT || 'artifacts/v2-component-reorder-review',
);
...
async function geometry(page, label) {
  const metrics = await page.evaluate(() => {
    ...
    return {
      ...
      selectCount: globalThis.document.querySelectorAll('select[name="destination"]').length,
  ...
  assert.equal(metrics.textareaCount, 0, `${label}: structural slice must not expose text editor`);
  assert.equal(metrics.selectCount, 1, `${label}: expected one stable position selector`);
  assert.ok(
...
async function previewOrder(page) {
  ...
  return frame.locator('[data-component-id]').evaluateAll((elements) => {
    const ids = elements.map((element) => element.getAttribute('data-component-id')).filter(Boolean);
    return [...new Set(ids)];
  });
}

function assertDestination(order, spec, label) {
  const componentId = spec.nodeId.replace(/^component:/, '');
  const targetIndex = order.indexOf(componentId);
  assert.ok(targetIndex >= 0, `${label}: target component missing from real renderer`);
  if (spec.destination === 'END_OF_PAGE') {
    assert.equal(targetIndex, order.length - 1, `${label}: component did not move to page end`);
    return;
  }
  const beforeComponentId = spec.destination.slice('BEFORE_COMPONENT:'.length);
  const siblingIndex = order.indexOf(beforeComponentId);
  assert.ok(siblingIndex >= 0, `${label}: destination sibling missing from real renderer`);
  assert.equal(siblingIndex, targetIndex + 1, `${label}: target is not directly before destination sibling`);
}
...
async function runReference(browser, spec) {
  ...
  try {
    ...
    const selector = page.locator('select[name="destination"]');
    await selector.focus();
    await page.keyboard.press(spec.keyboardKey);
    assert.equal(await selector.inputValue(), spec.destination);
    ...
    const previewComponentOrder = await previewOrder(page);
    assert.notDeepEqual(previewComponentOrder, baselineOrder);
    assertDestination(previewComponentOrder, spec, `${spec.referenceId}/preview`);
    ...
    const previewScreenshot = await capture(page, spec.referenceId, 'reorder-preview');
    ...
    const selectorAgain = page.locator('select[name="destination"]');
    await selectorAgain.focus();
    await page.keyboard.press(spec.keyboardKey);
    assert.equal(await selectorAgain.inputValue(), spec.destination);
    const secondNavigation = page.waitForNavigation({ waitUntil: 'networkidle' });
    await page.keyboard.press('Tab');
    await page.keyboard.press('Enter');
    ...
    const appliedOrder = await previewOrder(page);
    assertDestination(appliedOrder, spec, `${spec.referenceId}/applied`);
    const appliedScreenshot = await capture(page, spec.referenceId, 'reorder-applied');
    ...
    const redoneOrder = await previewOrder(page);
    assert.deepEqual(redoneOrder, appliedOrder);
    assertDestination(redoneOrder, spec, `${spec.referenceId}/redone`);
...
async function main() {
  ...
  const summary = {
    ...
    structuralOrderProofCount: references.length,
  ...
  assert.equal(summary.structuralOrderProofCount, 4);
  ...
  const manifest = {
    kind: 'hivenues-v2-component-reorder-visual-evidence',
    schemaVersion: 1,
  ...
  console.log('V2_COMPONENT_REORDER_VISUAL_EVIDENCE', JSON.stringify(summary));
}
