...
async function main() {
  ...
  try {
    ...
    await openButton(desktop, 'Page', 'First impression');
    ...
    await openButton(desktop, 'Activities', 'Friday Night Assembly');
    ...
    await gotoOk(sessionB, `/candidate-c/studio/${SLUG}`);
    await openButton(sessionB, 'Page', 'First impression');
    ...
    await openButton(mobile, 'Activities', 'Friday Night Assembly');
    ...
    fs.writeFileSync(path.join(OUTPUT_ROOT, 'manifest.json'), `${JSON.stringify({
      ...
      screenshots, geometry, accessibility, externalRequests, consoleErrors, expectedConflictDiagnostics, diagnostics, summary, proof,
