...
const OUTPUT = path.resolve(
  ROOT,
  process.env.V2_LOCAL_MEDIA_REVIEW_ROOT || 'artifacts/v2-local-media-authoring-review',
);
...
function assertRendererChanged(before, after, expectedAsset, expectedAlt, label) {
  ...
  assert.equal(after.complete, true, label + ': renderer image did not complete');
  assert.equal(after.naturalWidth, expectedAsset.width, label + ': decoded width mismatch');
  assert.equal(after.naturalHeight, expectedAsset.height, label + ': decoded height mismatch');
...
async function importLocalMediaByKeyboard(page, altText, label) {
  ...
  assert.equal(await form.count(), 1, label + ': expected one meaningful local import form');
  ...
  assert.match(await file.inputValue(), /local-hero\.gif$/, label + ': file chooser did not retain selection');
  ...
  assert.equal(activeText, 'Preview local image');
...
async function runReference(browser, spec) {
  ...
  const targetAlt = spec.referenceId + ' locally imported hero image';
  ...
  try {
    ...
    const baselineAssetCount = source.media.assets.length;
    ...
    const localForm = page.locator('form[data-local-media-import="meaningful"]');
    assert.equal(await localForm.count(), 1, spec.referenceId + ': local image import form missing');
    assert.equal(await localForm.locator('input[name="assetId"]').count(), 0);
    assert.equal(await localForm.locator('input[name="src"]').count(), 0);
    assert.equal(await localForm.locator('input[name="width"]').count(), 0);
    assert.equal(await localForm.locator('input[name="height"]').count(), 0);
    assert.equal(await localForm.locator('input[name="digestSha256"]').count(), 0);
    await localForm.scrollIntoViewIfNeeded();
    const editorGeometry = await geometry(page, spec.referenceId + '/editor');
    const editorAccessibility = await accessibility(page, spec.referenceId + '/editor');
    const editorScreenshot = await capture(page, spec.referenceId, 'editor');

    await importLocalMediaByKeyboard(page, targetAlt, spec.referenceId + '/first');
    const firstProposal = fixture.proposal();
    assert.equal(firstProposal.command.type, 'IMPORT_LOCAL_HERO_MEDIA');
    assert.equal(firstProposal.resolvedTarget.assetWasAdded, true);
    assert.equal(firstProposal.resolvedTarget.mediaType, 'image/gif');
    assert.equal(firstProposal.resolvedTarget.width, 4);
    assert.equal(firstProposal.resolvedTarget.height, 4);
    assert.match(firstProposal.resolvedTarget.digestSha256, /^[0-9a-f]{64}$/);
    const importedAsset = firstProposal.previewSource.media.assets.find(
      (asset) => asset.id === firstProposal.resolvedTarget.assetId,
    );
    assert.ok(importedAsset);
    assert.equal(firstProposal.previewSource.media.assets.length, baselineAssetCount + 1);
    ...
    assert.equal(fixture.diagnostics().ephemeralMediaBytes, LOCAL_GIF_BYTES.length);
    ...
    await importLocalMediaByKeyboard(page, targetAlt, spec.referenceId + '/second');
    const secondProposal = fixture.proposal();
    assert.equal(secondProposal.resolvedTarget.assetId, importedAsset.id);
    assert.equal(secondProposal.afterDigest, firstProposal.afterDigest);
    ...
    assertRendererChanged(
      ...
      importedAsset,
      targetAlt,
      spec.referenceId + '/applied',
    );
    assert.deepEqual(appliedRenderer, secondPreviewRenderer);
    const acceptedHero = fixture.session().draftSource.site.pages
      .find((candidate) => candidate.id === pageSource.id).components
      .find((component) => component.id === hero.id);
    assert.equal(acceptedHero.content.media.assetId, importedAsset.id);
    assert.equal(acceptedHero.content.media.alt, targetAlt);
    assert.equal(
      JSON.stringify(acceptedHero.content.media.treatment),
      JSON.stringify(baselineUsage.treatment),
    );
    assert.equal(fixture.session().draftSource.media.assets.length, baselineAssetCount + 1);
    ...
    assert.equal(fixture.diagnostics().ephemeralMediaEntries, 0);
    assert.equal(
      fixture.session().draftSource.media.assets.some((asset) => asset.id === importedAsset.id),
      false,
    );
    ...
    assert.equal(diagnostics.localMediaImportRequests, 2);
    ...
    assert.equal(diagnostics.ephemeralMediaEntries, 1);
    assert.equal(diagnostics.ephemeralMediaBytes, LOCAL_GIF_BYTES.length);
    ...
    return {
      ...
      assetSrc: importedAsset.src,
      digestSha256: firstProposal.resolvedTarget.digestSha256,
      targetAlt,
...
async function main() {
  ...
  const summary = {
    ...
    deterministicDerivedIdentityCount: new Set(references.map((item) => item.assetId)).size,
    assetAdmissionProofCount: references.length,
    byteDecodeProofCount: references.filter((item) => item.renderer.preview.naturalWidth === 4).length,
    discardReleaseProofCount: references.filter((item) => item.diagnostics.discards === 1).length,
    applyProofCount: references.length,
    undoReleaseProofCount: references.filter((item) => item.renderer.undo.src === item.renderer.before.src).length,
    redoReconstructionProofCount: references.filter((item) => item.renderer.redo.src === item.assetSrc).length,
  ...
  assert.equal(summary.deterministicDerivedIdentityCount, 1);
  assert.equal(summary.assetAdmissionProofCount, 4);
  assert.equal(summary.byteDecodeProofCount, 4);
  assert.equal(summary.discardReleaseProofCount, 4);
  assert.equal(summary.applyProofCount, 4);
  assert.equal(summary.undoReleaseProofCount, 4);
  assert.equal(summary.redoReconstructionProofCount, 4);
  ...
  const manifest = {
    kind: 'hivenues-v2-local-media-authoring-visual-evidence',
    schemaVersion: 1,
  ...
  console.log('V2_LOCAL_MEDIA_AUTHORING_VISUAL_EVIDENCE', JSON.stringify(summary));
}
