...
async function main() {
  ...
  try {
    ...
    assert.equal(await page.getByRole('link', { name: 'Create a place' }).count(), 1);
    await page.getByRole('link', { name: 'Create a place' }).click();
    await page.waitForURL((url) => url.pathname === '/candidate-c/new');
    await capture(page, '02-new-host-form', 'new-host-form');
    ...
    const fields = {
      ...
      activityDescription: 'A scheduled gathering entered entirely through the operator form.',
    ...
    await capture(page, '05-urgent-review', 'urgent-review');
    await Promise.all([
    ...
    await capture(page, '06-cancelled-public', 'cancelled-public');
