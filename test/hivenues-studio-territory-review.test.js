...
test('Studio exposes transient semantic page review without replacing the canvas or durable host state', async () => {
  ...
  const studio = await request(app).get('/hivenues/studio/lantern-relay').expect(200);
  ...
  const review = await request(app)
    .get('/hivenues/studio/lantern-relay/review?surface=stories-index')
    .set('HX-Request', 'true')
    .expect(200);
  ...
  assert.match(review.text, /src="\/hivenues\/studio\/lantern-relay\/preview\/stories"/);
...
test('Studio semantic review has ordinary full Preview fallback and rejects unknown surfaces', async () => {
  ...
  await request(app)
    .get('/hivenues/studio/lantern-relay/review?surface=stories-index')
    .expect(303)
    .expect('Location', '/hivenues/studio/lantern-relay/preview/stories');

  const canvas = await request(app)
    .get('/hivenues/studio/lantern-relay/review?surface=canvas')
    .set('HX-Request', 'true')
    .expect(200);
  ...
  await request(app)
    .get('/hivenues/studio/lantern-relay/review?surface=story-detail:story-lantern-roofline-001')
    .set('HX-Request', 'true')
    .expect(400)
...
test('v1 Studio review exposes only derivable territory pages and fabricates no v2 surfaces', async () => {
  const northline = seedHiVenuesHosts().find((host) => host.identity.slug === 'northline-hall');
  ...
  await request(app)
    .get('/hivenues/studio/northline-hall/review?surface=stories-index')
    .set('HX-Request', 'true')
    .expect(400);
});
