...
const { buildTerritoryProjection } = require('../src/product/territory');
const { upgradeGraphToV2 } = require('../src/product/territory-authoring-router');

function northlineV1() {
  return structuredClone(seedHiVenuesHosts().find((host) => host.identity.slug === 'northline-hall'));
}

function storeFor(host = northlineV1()) {
  return new HiVenuesStore({ hosts: [host], now: () => Date.parse('2026-09-16T17:00:00Z') });
}
...
test('v1 territory upgrade is explicit, Working-only and content-empty', async () => {
  ...
  const studio = await request(app).get('/hivenues/studio/northline-hall').expect(200);
  assert.match(studio.text, /href="\/hivenues\/studio\/northline-hall\/territory-content"/);

  const hub = await request(app).get('/hivenues/studio/northline-hall/territory-content').expect(200);
  assert.match(hub.text, /Enable territory content in Working/);

  await request(app)
    .post('/hivenues/studio/northline-hall/territory-content/enable')
    .type('form')
...
test('ordinary operator can author profiles, stories and a gallery without leaking Working to Live', async () => {
  ...
  await request(app)
    .post('/hivenues/studio/northline-hall/territory-content/profile/new')
    .type('form')
    .send({
  ...
  await request(app)
    .post('/hivenues/studio/northline-hall/territory-content/story/new')
    .type('form')
    .send({
      ...concurrency(snapshot),
      kind: 'essay',
  ...
  const preview = await request(app)
    .get(`/hivenues/studio/northline-hall/preview/stories/${storySlug}`)
    .expect(200);
  assert.match(preview.text, /Working-only story copy/);
  await request(app).get(`/hivenues/northline-hall/stories/${storySlug}`).expect(404);
  assert.equal(store.publicSnapshot('northline-hall').draft.schemaVersion, 1);

  const mediaA = snapshot.draft.media[0].id;
  await request(app)
    .post('/hivenues/studio/northline-hall/territory-content/gallery')
    .type('form')
    .send({
      ...concurrency(snapshot),
  ...
  await request(app)
    .post(`/hivenues/studio/northline-hall/territory-content/story/${storyId}`)
    .type('form')
    .send({
      ...concurrency(snapshot),
      kind: 'essay',
  ...
  await request(app)
    .post(`/hivenues/studio/northline-hall/territory-content/profile/${profile.id}/delete`)
    .type('form')
    .send(concurrency(snapshot))
    .expect(409)
  ...
  await request(app)
    .post('/hivenues/studio/northline-hall/release')
    .type('form')
    .send(concurrency(snapshot))
    .expect(303);
  ...
  const publicStory = await request(app)
    .get(`/hivenues/northline-hall/stories/${stableStorySlug}`)
    .expect(200);
...
test('operator can author a short-form Update and semantic navigation without fabricating empty routes', async () => {
  ...
  const updateForm = await request(app)
    .get('/hivenues/studio/northline-hall/territory-content/update/new')
    .expect(200);
  assert.match(updateForm.text, /Share a quick Update/);

  await request(app)
    .post('/hivenues/studio/northline-hall/territory-content/update/new')
    .type('form')
    .send({
      ...concurrency(snapshot),
  ...
  const preview = await request(app)
    .get(`/hivenues/studio/northline-hall/preview/stories/${stableUpdateSlug}`)
    .expect(200);
  assert.match(preview.text, /Doors are open/);
  await request(app).get(`/hivenues/northline-hall/stories/${stableUpdateSlug}`).expect(404);
  ...
  const navigationForm = await request(app)
    .get('/hivenues/studio/northline-hall/territory-content/navigation')
    .expect(200);
  ...
  await request(app)
    .post('/hivenues/studio/northline-hall/territory-content/navigation')
    .type('form')
    .send({
      ...concurrency(snapshot),
  ...
  await request(app)
    .post(`/hivenues/studio/northline-hall/territory-content/update/${stableUpdateId}`)
    .type('form')
    .send({
      ...concurrency(snapshot),
  ...
  const publicUpdate = await request(app)
    .get(`/hivenues/northline-hall/stories/${stableUpdateSlug}`)
    .expect(200);
