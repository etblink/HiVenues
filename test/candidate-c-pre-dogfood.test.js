...
const { buildCandidateCHostFromInput, localDateTimeToOffsetIso } = require('../src/candidate-c/admission');
...
const { parseArgs } = require('../scripts/candidate-c-dogfood');
...
function input(overrides = {}) {
  return {
    ...
    summary: 'A bounded operator-created host used to prove Candidate C fresh-host admission without source editing.',
...
function tempState(t) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'hivenues-candidate-c-'));
  t.after(() => fs.rmSync(dir, { recursive: true, force: true }));
  return path.join(dir, 'state.json');
}

function build(raw = input()) {
  return buildCandidateCHostFromInput(raw, { randomUUID: () => '11111111-1111-4111-8111-111111111111' });
}
...
test('public-ingress mode fails closed, issues a bounded session, rejects foreign-origin mutations, and never persists the secret', async (t) => {
  ...
  await agent.get('/candidate-c').set('Host', 'dogfood.test').expect(303).expect('Location', '/__dogfood/access');
  ...
  await agent.get('/candidate-c').set('Host', 'dogfood.test').expect(200);
  await agent.post('/candidate-c/new')
    .set('Host', 'dogfood.test')
