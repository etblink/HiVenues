...
function fixture(t) {
  ...
  return {
    statePath,
    store: new FileHiVenuesStore({ statePath, now: () => Date.parse('2026-09-16T17:30:00Z') }),
  };
}
...
test('territory-authored objects and navigation survive restart without leaking Working before Release', async (t) => {
  ...
  await post(app, `/hivenues/studio/${slug}/territory-content/story/new`, snapshot, {
    kind: 'essay',
    title: 'After the room restarts',
    dek: 'A durable territory-authoring proof.',
    body: 'This story must survive a full FileHiVenuesStore restart with stable identity and remain Working-only until Release.',
  ...
  store = new FileHiVenuesStore({ statePath, now: () => Date.parse('2026-09-16T18:00:00Z') });
  app = appFor(store);
  ...
  await request(app)
    .get(`/hivenues/studio/${slug}/preview/stories/${beforeRestart.updateSlug}`)
    .expect(200)
    .expect(/restart proof begins/i);
  await request(app)
    .get(`/hivenues/studio/${slug}/preview/stories/${beforeRestart.storySlug}`)
    .expect(200)
    .expect(/survive a full FileHiVenuesStore restart/i);
  await request(app).get(`/hivenues/${slug}/stories/${beforeRestart.updateSlug}`).expect(404);
  await request(app).get(`/hivenues/${slug}/stories/${beforeRestart.storySlug}`).expect(404);

  await post(app, `/hivenues/studio/${slug}/release`, restarted);
  ...
  store = new FileHiVenuesStore({ statePath, now: () => Date.parse('2026-09-16T18:30:00Z') });
...
test('guided creator provisions a file-backed Working host that survives restart without leaking Live state', async (t) => {
  ...
  await request(app)
    ...
    .expect(303)
    .expect('Location', '/hivenues/studio/lantern-relay?created=1');
  ...
  const restartedStore = new FileHiVenuesStore({
    statePath,
  ...
  await request(appFor(restartedStore))
    .get('/hivenues/studio/lantern-relay')
    .expect(200)
