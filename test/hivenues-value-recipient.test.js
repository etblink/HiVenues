...
function fixture() {
  const store = new HiVenuesStore({ now: () => Date.parse('2026-09-18T16:00:00Z') });
  const app = express();
...
test('Studio value-recipient edit is Working-only until Release and creates host-native support Voice', async () => {
  ...
  const response = await request(app)
    .post('/hivenues/studio/' + slug + '/value-recipient')
    .set('HX-Request', 'true')
    .type('form')
    .send({
      ...tokens(store, slug),
...
test('released value recipient creates only a host-native support link, never an inline transfer control', async () => {
  ...
  assert.match(publicPage.text, /href="\/hivenues\/northline-hall\/support"/);
