...
test('UX-1B visual fixture activates beneficiary and funds-claim states without mutations', async () => {
  ...
  const page = await request(fixture.app).get('/community/threads').set('cookie', `hive_bar_session=${fixture.token}`).expect(200);
  ...
  const merchantPage = await request(fixture.app).get('/community/threads').set('cookie', `hive_bar_session=${fixture.merchantToken}`).expect(200);
  ...
  const unavailablePage = await request(fixture.app).get('/community/threads').set('cookie', `hive_bar_session=${fixture.merchantToken}`).expect(200);
  ...
  await request(fixture.app).post('/api/social/preflight/thread').send({ body: 'must not prepare' }).expect(405)
    .expect(({ body }) => assert.equal(body.error.code, 'UX_1B_VISUAL_MUTATION_FORBIDDEN'));
  assert.deepEqual(fixture.mutationAttempts, [{ method: 'POST', path: '/api/social/preflight/thread' }]);
});
...
test('UX-1B pinned-Chromium contract covers composers and new conditional economic UI', () => {
  ...
  for (const scenario of ['community-post-active','thread-active','nested-reply-active','public-wall-active','private-message-active','threads-funds-claim-ready','threads-funds-claim-unavailable']) assert.match(capture, new RegExp(scenario));
