...
const { compositionRegistry } = require('../src/product/present');
const { HiVenuesStore } = require('../src/product/store');
const { mechanicRegistry } = require('../src/product/model');

function appFixture(store = new HiVenuesStore({ now: () => Date.parse('2026-09-14T17:40:00Z') })) {
  const app = express();
...
test('HiVenues has one canonical graph with durable identity and materially distinct composition trees', async () => {
  ...
  const poster = await request(app).get('/hivenues/northline-hall').expect(200);
  const editorial = await request(app).get('/hivenues/nova-ashby').expect(200);
...
test('public site reads immutable live Release while Studio edits a newer server-owned draft', async () => {
  ...
  const publicBeforeRelease = await request(app).get('/hivenues/northline-hall').expect(200);
  ...
  const draftPreview = await request(app).get('/hivenues/studio/northline-hall/preview').expect(200);
  ...
  const publicAfterRelease = await request(app).get('/hivenues/northline-hall').expect(200);
...
test('ordinary HTMX edit is one server revision with coherent OOB canvas/status update and stale writes fail closed', async () => {
  ...
  const first = await request(app)
    .post('/hivenues/studio/northline-hall/activity')
    .set('HX-Request', 'true')
    .type('form')
    .send({
      expectedRevision: 1,
...
test('canonical Activity route, accountless RSVP and ICS remain useful without Hive', async () => {
  const { app, store } = appFixture();
  const activityUrl = '/hivenues/northline-hall/activities/friday-night-assembly';
...
test('Voice maps host-native terms to typed mechanics without conflating account follow and community membership', async () => {
  ...
  const publicPage = await request(app).get('/hivenues/northline-hall').expect(200);
  assert.doesNotMatch(publicPage.text, /blockchain/i);

  const consequence = await request(app).get('/hivenues/northline-hall/consequence/follow_account').expect(200);
...
test('Undo and Release restore create new revisions and never silently time-travel live state', () => {
  const store = new HiVenuesStore({ now: () => Date.parse('2026-09-14T17:40:00Z') });
...
test('Studio is server-rendered, current-revision aware and uses one transient-only client island', async () => {
  ...
  assert.match(studio.text, /src="\/js\/hivenues-studio\.js"/);
