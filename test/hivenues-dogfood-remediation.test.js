...
test('dogfood remediation: visitor-visible content stays draft-only until an explicit Release and survives restart', async (t) => {
  ...
  await request(app).post(`/hivenues/studio/${slug}/content`).type('form').send({
    ...tokens(store, slug),
  ...
  const restartedBeforeRelease = new ProvisioningFileHiVenuesStore({ statePath, mediaRoot });
...
test('dogfood remediation: zero-event host can add an Activity and Offering later without developer intervention', async (t) => {
  ...
  const built = buildHiVenuesHostFromInput(freshHostInput(), { randomUUID: () => '33333333-3333-4333-8333-333333333333' });
  ...
  await request(app).post(`/hivenues/studio/${slug}/activity/new`).type('form').send({
    ...tokens(store, slug),
  ...
  await request(app).post(`/hivenues/studio/${slug}/offer/new`).type('form').send({
    ...tokens(store, slug),
...
test('dogfood remediation: local image import derives bytes, keeps hero/logo roles separate, rejects stale authority and survives restart', async (t) => {
  ...
  await request(app).post(`/hivenues/studio/${slug}/media-import`)
  ...
  assert.match(hero.asset.path, /^\/hivenues\/media\/local\/harbor-and-hearth\/[a-f0-9]{24}\.png$/);
...
test('dogfood remediation: media alt and caption stay bounded data and render escaped in operator review', async (t) => {
  ...
  const library = await request(app).get(`/hivenues/studio/${slug}/media-library`).expect(200);
...
test('dogfood remediation: Studio exposes canvas-first draft authoring, dedicated review and ordinary empty-state paths', async (t) => {
  const { store, app } = tempRuntime(t);
  const built = buildHiVenuesHostFromInput(freshHostInput(), { randomUUID: () => '44444444-4444-4444-8444-444444444444' });
  assert.equal(built.ok, true);
  assert.equal(store.createHost(built.graph).ok, true);
  const slug = built.graph.identity.slug;
  ...
  assert.match(studio.text, new RegExp(`/hivenues/studio/${slug}/preview`));
  assert.doesNotMatch(studio.text, /cc-complete-preview/);
  assert.doesNotMatch(studio.text, /cc-full-preview-frame/);

  const draftPreview = await request(app).get(`/hivenues/studio/${slug}/preview`).expect(200);
