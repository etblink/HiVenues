...
function storeFixture(t) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'hivenues-candidate-c-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const statePath = path.join(directory, 'candidate-c-state.json');
  return {
    directory,
    statePath,
    store: new FileCandidateCStore({ statePath, now: () => Date.parse('2026-09-14T20:00:00Z') }),
  };
}
...
test('independent stores reject stale revision and same-revision digest mismatch', (t) => {
  ...
  const storeB = new FileCandidateCStore({ statePath });
...
test('durable state fails closed for malformed, unsupported and tampered persistence', (t) => {
  ...
  assert.throws(
    () => new FileCandidateCStore({ statePath }).snapshot('northline-hall'),
    (error) => error.code === 'CANDIDATE_C_STATE_PARSE_FAILED'
  );
  ...
  assert.throws(
    () => new FileCandidateCStore({ statePath }).snapshot('northline-hall'),
    (error) => error.code === 'CANDIDATE_C_STATE_VERSION_UNSUPPORTED'
  );

  fs.rmSync(statePath, { force: true });
  const fresh = new FileCandidateCStore({ statePath });
  ...
  assert.throws(
    () => new FileCandidateCStore({ statePath }).snapshot(envelope.state.workspaces[0].slug),
    (error) => error.code === 'CANDIDATE_C_INVALID_PERSISTED_STATE'
  );

  fs.rmSync(statePath, { force: true });
  const freshRelease = new FileCandidateCStore({ statePath });
  ...
  assert.throws(
    () => new FileCandidateCStore({ statePath }).snapshot(releaseEnvelope.state.workspaces[0].slug),
    (error) => error.code === 'CANDIDATE_C_INVALID_PERSISTED_STATE'
  );
});
...
test('Studio emits revision plus digest tokens and durable router rejects a missing digest', async (t) => {
  ...
  await request(app)
    .post('/candidate-c/studio/northline-hall/tagline')
    .set('HX-Request', 'true')
    .type('form')
