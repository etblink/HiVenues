...
const { deriveUrgentClosure, diffGraphPaths, graphPaths, verifyUrgentOperation, verifyUrgentRelease } = require('../src/candidate-c/urgent');
...
function memoryStore() {
  return new CandidateCStore({ now: () => FIXED_NOW });
}

function fileStore(t) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'hivenues-candidate-c-urgent-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const statePath = path.join(directory, 'candidate-c-state.json');
  return { statePath, store: new FileCandidateCStore({ statePath, now: () => FIXED_NOW }) };
}
...
test('durable store persists urgent operations, releases and provenance across restart and validates them fail-closed', (t) => {
  ...
  const restartedForReview = new FileCandidateCStore({ statePath });
  ...
  assert.throws(() => new FileCandidateCStore({ statePath }).snapshot(SLUG), (error) => error.code === 'CANDIDATE_C_INVALID_PERSISTED_STATE');
...
test('ordinary draft status edit is a normal revision and the next whole-host release carries it', async () => {
  ...
  await request(app)
    .post(`/candidate-c/studio/${SLUG}/activity-status`)
    .set('HX-Request', 'true')
    .type('form')
...
test('Studio review distinguishes changed, retained-live and unpublished-draft state and publishes through HTTP', async () => {
  ...
  const inspector = await request(app).get(`/candidate-c/studio/${SLUG}/inspect?resource=activity:${ACTIVITY}`).expect(200);
  assert.match(inspector.text, /Change the live status now/);
  assert.match(inspector.text, /name="lifecycle"/);

  const compose = await request(app).get(`/candidate-c/studio/${SLUG}/urgent?activity=${ACTIVITY}`).expect(200);
  ...
  const created = await request(app)
    .post(`/candidate-c/studio/${SLUG}/urgent`)
    .type('form')
    .send({ activityId: ACTIVITY, lifecycle: 'cancelled', statusNote: 'Power is out on the block.' })
    .expect(303);
  const reviewPath = created.headers.location.replace(/^\/candidate-c/, '');
  const operationId = reviewPath.split('/').pop();

  const review = await request(app).get(`/candidate-c${reviewPath}`).expect(200);
  ...
  await request(app)
    .post(`/candidate-c/studio/${SLUG}/urgent/${operationId}/publish`)
    .type('form')
  ...
  const studio = await request(app).get(`/candidate-c/studio/${SLUG}?released=${releasedId}&urgent=1`).expect(200);
  ...
  const publicActivity = await request(app).get(`/candidate-c/${SLUG}/activities/friday-night-assembly`).expect(200);
  assert.match(publicActivity.text, /data-rsvp-closed/);
  assert.doesNotMatch(publicActivity.text, /id="cc-rsvp-name"/);
  await request(app).post(`/candidate-c/${SLUG}/activities/friday-night-assembly/rsvp`).set('HX-Request', 'true').type('form').send({ name: 'Late' }).expect(409).expect(/Nothing was recorded/);
  assert.equal(store.diagnostics().local.rsvps, 0);

  const ics = await request(app).get(`/candidate-c/${SLUG}/activities/friday-night-assembly/calendar.ics`).expect(200);
  ...
  const history = await request(app).get(`/candidate-c/studio/${SLUG}/release`).expect(200);
...
test('urgent compose refuses draft-only activities and review fails closed when live moved', async () => {
  const store = memoryStore();
  const app = appFor(store);
  const missing = await request(app).get(`/candidate-c/studio/${SLUG}/urgent?activity=activity-not-live`).expect(200);
  assert.doesNotMatch(missing.text, /not on the live site yet\./);
  await request(app).post(`/candidate-c/studio/${SLUG}/urgent`).type('form').send({ activityId: 'activity-not-live', lifecycle: 'cancelled' }).expect(400);

  const created = await request(app).post(`/candidate-c/studio/${SLUG}/urgent`).type('form').send({ activityId: ACTIVITY, lifecycle: 'cancelled' }).expect(303);
  ...
  const review = await request(app).get(`/candidate-c/studio/${SLUG}/urgent/${operationId}`).expect(200);
...
test('all three families render status truthfully on home and activity pages', async () => {
  ...
  for (const [slug, activityId, activitySlug] of cases) {
    ...
    const activity = await request(app).get(`/candidate-c/${slug}/activities/${activitySlug}`).expect(200);
...
test('E-PROVENANCE-1B: HTTP compose refuses a no-op calmly and review shows only the exact change', async () => {
  ...
  await request(app)
    ...
    .expect(303)
    .expect('Location', `/candidate-c/studio/${SLUG}/urgent?activity=${ACTIVITY}&nochange=1`);
  const compose = await request(app).get(`/candidate-c/studio/${SLUG}/urgent?activity=${ACTIVITY}&nochange=1`).expect(200);
...
test('E-PROVENANCE-1A: import re-derives every urgent operation and Release and fails closed on any tampered derived field', (t) => {
  ...
  for (const [label, tamper] of cases) {
    ...
    assert.throws(
      () => new FileCandidateCStore({ statePath: ids.statePath }).snapshot(SLUG),
  ...
  const ids = seedReleased(t);
  const restarted = new FileCandidateCStore({ statePath: ids.statePath });
