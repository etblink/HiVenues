...
function configFrom(overrides = {}) {
  return loadConfig({ NODE_ENV: 'test', HIVE_WRITE_MODE: 'disabled', ...overrides }, { loadDotenv: false });
}
...
test('allows only explicitly account-scoped controlled mode and still rejects production mode', () => {
  const controlled = configFrom({ HIVE_WRITE_MODE: 'controlled', HIVE_CONTROLLED_ACCOUNTS: 'etblink, etblink' });
  ...
  assert.equal(configFrom({ HIVE_SIGNER_MODE: 'keychain' }).hive.signerMode, 'keychain');
  assert.throws(() => configFrom({ HIVE_SIGNER_MODE: 'browser-wallet' }), /Invalid option/);
  assert.throws(() => configFrom({ HIVE_WRITE_MODE: 'controlled' }), /Controlled mode requires at least one explicitly allowlisted Hive account/);
  assert.throws(() => configFrom({ HIVE_CONTROLLED_ACTIONS: 'post,transfer' }), /Invalid controlled action: transfer/);
  assert.throws(() => configFrom({ HIVE_WRITE_MODE: 'production' }), /Production write mode is not authorized before the V1 release gate/);
});

test('explicit Pay is beta + Keychain only and does not require controlled payer/action allowlists', () => {
  const config = configFrom({
    HIVE_WRITE_MODE: 'beta', HIVE_SIGNER_MODE: 'keychain', HIVE_CONTROLLED_ACCOUNTS: '', HIVE_CONTROLLED_ACTIONS: '',
    HIVE_PAYMENT_ENABLED: 'true', HIVE_PAYMENT_RECEIPT_DB_PATH: ':memory:',
  });
  assert.equal(config.payments.enabled, true);
  assert.deepEqual(config.hive.controlledAccounts, []);
  assert.deepEqual(config.hive.controlledActions, []);
  assert.throws(() => configFrom({ HIVE_PAYMENT_ENABLED: 'true' }), /Enabled payment requires exact beta write mode/);
...
test('validates the canonical M4 wall fee and both normalized exclusion layers', () => {
  const config = configFrom({ HIVE_WALL_DEFAULT_FEE: '1.000 HBD', HIVE_GLOBAL_WALL_EXCLUSIONS: 'rewardbot, spammer, rewardbot', HIVE_MESSAGE_HISTORY_PAGE_SIZE: '50' });
  assert.equal(config.hive.defaultWallFee, '1.000 HBD');
...
test('rejects insecure or credential-bearing RPC URLs and normalizes duplicates', () => {
  assert.throws(() => configFrom({ HIVE_RPC_NODES: 'http://api.hive.blog' }), /credential-free HTTPS URL/);
  assert.throws(() => configFrom({ HIVE_RPC_NODES: 'https://user:secret@example.com' }), /credential-free HTTPS URL/);
  const config = configFrom({ HIVE_RPC_NODES: ' https://api.hive.blog,https://api.hive.blog,https://api.openhive.network ' });
  assert.deepEqual(config.hive.rpcNodes, ['https://api.hive.blog', 'https://api.openhive.network']);
