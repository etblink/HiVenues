...
function appFixture(store = new CandidateCStore({ now: () => Date.parse('2026-09-14T23:45:00Z') })) {
  const app = express();
...
test('Offer and Look edits use revision plus digest, update the canonical graph and keep live Release immutable', async () => {
  ...
  await request(app)
    .post('/candidate-c/studio/harbor-and-hearth/offer')
    .set('HX-Request', 'true')
    .type('form')
    .send({
  ...
  await request(app)
    .post('/candidate-c/studio/harbor-and-hearth/look')
    .set('HX-Request', 'true')
    .type('form')
...
test('Connect edits visitor contact only and cannot mutate Hive or media bindings', async () => {
  ...
  await request(app)
    .post('/candidate-c/studio/northline-hall/connect')
    .set('HX-Request', 'true')
    .type('form')
...
test('durable D mutation rejects stale dual-token state and survives restart', async (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'hivenues-candidate-c-studio-d-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const statePath = path.join(directory, 'state.json');
  const store = new FileCandidateCStore({ statePath, now: () => Date.parse('2026-09-14T23:45:00Z') });
  const { app } = appFixture(store);
  const before = store.snapshot('harbor-and-hearth');

  await request(app)
    .post('/candidate-c/studio/harbor-and-hearth/connect')
    .set('HX-Request', 'true')
    .type('form')
