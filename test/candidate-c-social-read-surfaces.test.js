...
function appWith({ rpc = new RealShapedSocialRpc(), store = new CandidateCStore(), bindings = socialBindings() } = {}) {
...
test('partial, unavailable, disconnected, and degraded social states stay explicit', async () => {
  ...
  const disconnectedApp = createDogfoodApp({ store: new CandidateCStore() });
...
test('social hub and member profile use Live host Direction rather than unreleased Working state', async () => {
  ...
  const hub = await request(app).get('/candidate-c/northline-hall/community/updates').expect(200);
  const member = await request(app).get('/candidate-c/northline-hall/community/people/juniper-lane').expect(200);
...
test('accepted discussion surface discovers the read-only updates and people layer', async () => {
  const { app } = appWith();
  for (const host of HOSTS) {
    const response = await request(app).get(`/candidate-c/${host.slug}/community`).expect(200);
    assert.match(response.text, new RegExp(`/candidate-c/${host.slug}/community/updates`));
  }
});

test('unknown hosts and unknown member profiles remain ordinary 404s', async () => {
  const { app } = appWith();
  await request(app).get('/candidate-c/not-a-host/community/updates').expect(404);
  await request(app).get('/candidate-c/northline-hall/community/people/not-found').expect(404);
  await request(app).get('/candidate-c/northline-hall/community/people/INVALID%20NAME').expect(404);
});
