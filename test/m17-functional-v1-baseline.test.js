...
test('functional V1 remains pre-final while successor routing advances independently', () => {
  ...
  assert.deepEqual(summary, {
    ...
    finalRelease: false,
    successorRouting: 'HV7_JUNIPER_WORKS_ARCHITECTURE_CONFRONTATION__READ_ONLY',
  });
});
...
test('frozen Juniper second-nominee routing coexists with unchanged production safety boundary', () => {
  ...
  assert.match(readme, /24 authentic product requirements are frozen/i);
  assert.match(readme, /next operation is a read-only architecture confrontation/i);
  ...
  for (const source of [roadmap, index]) {
    ...
    assert.match(source, /^NEXT_OPERATION = HV7_JUNIPER_WORKS_ARCHITECTURE_CONFRONTATION__READ_ONLY$/m);
