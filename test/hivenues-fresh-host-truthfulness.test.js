...
test('fresh-host admission permits a truthful zero-activity initial state', () => {
  const result = buildHiVenuesHostFromInput(input(), { randomUUID: () => '22222222-2222-4222-8222-222222222222' });
...
test('optional activity is all-or-none and refuses partial fabricated placeholders', () => {
  const partial = buildHiVenuesHostFromInput(input({ activityTitle: 'Unverified event' }));
...
test('fresh host stays working-only across restart until an explicit first Release', async (t) => {
  ...
  const publicPage = await request(restartedApp).get('/hivenues/truthful-bar').expect(200);
  ...
  const releasedIndex = await request(restartedApp).get('/hivenues/').expect(200);
  assert.match(releasedIndex.text, /Truthful Bar/);

  const secondRestart = new ProvisioningFileHiVenuesStore({ statePath });
...
test('durable state rejects mixed unpublished/live publication pointers', async (t) => {
  ...
  assert.throws(
    () => new ProvisioningFileHiVenuesStore({ statePath }).snapshot('truthful-bar'),
