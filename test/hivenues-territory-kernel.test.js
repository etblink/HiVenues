...
test('Territory Kernel preserves the exact frozen v1 graph digest and does not inject v2 fields', () => {
  const northline = seedHiVenuesHosts()[0];
...
test('same semantic host can change Direction recipe without changing semantic object identity', () => {
  ...
  assert.equal(templateForSurface(projections.poster, posterActivity), 'hivenues/compositions/poster-activity');
  assert.equal(templateForSurface(projections.editorial, editorialActivity), 'hivenues/compositions/editorial-activity');
  assert.equal(templateForSurface(projections.hospitality, hospitalityActivity), 'hivenues/compositions/hospitality-activity');
});
