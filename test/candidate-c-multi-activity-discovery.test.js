...
test('every released relevant Activity remains discoverable when the featured Activity is completed', async () => {
  for (const scenario of CASES) {
    ...
    const initial = await request(app).get(`/candidate-c/${scenario.slug}`).expect(200);
    ...
    const home = await request(app).get(`/candidate-c/${scenario.slug}`).expect(200);
    ...
    assert.match(home.text, new RegExp(escapeRegex(`/candidate-c/${scenario.slug}/activities/${added.secondSlug}`)));

    const detail = await request(app)
      .get(`/candidate-c/${scenario.slug}/activities/${added.secondSlug}`)
      .expect(200);
...
test('working preview uses the same featured/remainder discovery contract without exposing it publicly before Release', async () => {
  ...
  assert.match(preview.text, new RegExp(escapeRegex(`/candidate-c/studio/${slug}/preview/activities/${added.secondSlug}`)));
