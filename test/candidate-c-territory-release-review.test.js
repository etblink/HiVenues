...
function storeForNorthline() {
  ...
  return new CandidateCStore({ hosts: [host], now: () => Date.parse('2026-09-16T18:00:00Z') });
}
...
test('release review truthfully enumerates unpublished Territory v2 changes', async () => {
  ...
  await post(app, `/candidate-c/studio/${slug}/release`, snapshot);
  const postRelease = await request(app).get(`/candidate-c/studio/${slug}/release`).expect(200);
