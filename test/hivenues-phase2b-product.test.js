...
function appFixture(store = new HiVenuesStore({ now: () => Date.parse('2026-09-14T22:00:00Z') })) {
  const app = express();
...
test('Phase 2B exposes three genuinely different composition trees over the same graph contract', async () => {
  ...
  const hospitality = await request(app).get('/hivenues/harbor-and-hearth').expect(200);
...
test('admitted Media records stable semantic identity and exact local asset provenance while bootstrap fallback remains intact', async () => {
  ...
  const page = await request(app).get('/hivenues/harbor-and-hearth').expect(200);
  assert.match(page.text, /class="cc-admitted-media cc-admitted-media--hospitality"/);
  assert.match(page.text, /src="\/hivenues\/media\/harbor-hearth-table\.svg"/);
...
test('Harbor Studio and public Release share one composition and one canonical Media object', async () => {
  ...
  const publicPage = await request(app).get('/hivenues/harbor-and-hearth').expect(200);
...
test('hospitality is a real Direction target and preserves canonical Host, Activity, Offer and Media identities', () => {
  ...
  const hydrated = HiVenuesStore.fromState(store.exportState(), { now: () => Date.parse('2026-09-14T22:00:00Z') });
...
test('admitted Media survives durable restart and live Release remains immutable across later focal edits', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'hivenues-hivenues-media-'));
  const statePath = path.join(directory, 'state.json');
  const now = () => Date.parse('2026-09-14T22:00:00Z');

  try {
    const first = new FileHiVenuesStore({ statePath, now });
    ...
    const restarted = new FileHiVenuesStore({ statePath, now });
    ...
    const finalRestart = new FileHiVenuesStore({ statePath, now });
