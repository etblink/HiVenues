...
const { applyReadConsistencyHardening, socialOperationEquivalent } = require('../src/hive/read-consistency');
...
test('M16.7 binds first-party CSS and JavaScript URLs to exact runtime bytes', () => {
  ...
  assert.ok(pay.includes("assetUrl('/js/pay-tab.js')"));
...
test('M16.7 keeps new direct observation methods inside the read-only RPC allowlist', () => {
  assert.equal(assertReadOnlyRpcMethod('condenser_api', 'get_active_votes'), 'condenser_api.get_active_votes');
  assert.equal(assertReadOnlyRpcMethod('condenser_api', 'get_content'), 'condenser_api.get_content');
});
...
test('M16.7 normalizes tuple and appbase social operations for exact transaction matching', () => {
  assert.equal(
    socialOperationEquivalent(
      ...
      { type: 'vote_operation', value: { voter: 'etblink', author: 'fartman69', permlink: 'beta-post', weight: 100 } },
    ),
    true,
  );
});
...
test('M16.7 observes a social write by exact transaction id and operation before indexed reads', async () => {
  ...
  const operation = ['vote', { voter: 'etblink', author: 'fartman69', permlink: 'beta-post', weight: 100 }];
  const hiveReads = {
    rpcPool: { async call() { throw new Error('indexed fallback should not be used when the transaction id is present'); } },
    async getTransaction(id) {
      assert.equal(id, transactionId);
      return { transaction_id: transactionId, block_num: 456, operations: [{ type: 'vote_operation', value: { voter: 'etblink', author: 'fartman69', permlink: 'beta-post', weight: 100 } }] };
    },
    async getPostWithComments() { return { post: null, comments: [], profiles: {} }; },
    async observeSocialOperation() { return false; },
  };
  applyReadConsistencyHardening(hiveReads);
  const observed = await hiveReads.observeSocialOperation({ transactionId, operations: [operation] });
...
test('M16.7 falls back to direct active-vote lookup when Keychain provides no transaction id', async () => {
  const calls = [];
  const hiveReads = {
    rpcPool: { async call(api, method, params) { calls.push({ api, method, params }); return [{ voter: 'etblink', percent: -100, rshares: '0' }]; } },
    async getTransaction() { throw new Error('transaction lookup should not run without an id'); },
    async getPostWithComments() { return { post: null, comments: [], profiles: {} }; },
    async observeSocialOperation() { return false; },
  };
  applyReadConsistencyHardening(hiveReads);
  const observed = await hiveReads.observeSocialOperation({ transactionId: null, operations: [['vote', { voter: 'etblink', author: 'fartman69', permlink: 'beta-post', weight: -100 }]] });
  assert.equal(observed, true);
  assert.deepEqual(calls, [{ api: 'condenser_api', method: 'get_active_votes', params: ['fartman69', 'beta-post'] }]);
});

test('M16.7 refreshes post vote counts from direct active votes while preserving page availability', async () => {
  const baseDiscussion = { post: { author: 'fartman69', permlink: 'beta-post', positiveVotes: 0, negativeVotes: 0 }, comments: [], profiles: {} };
  const hiveReads = {
    rpcPool: { async call(api, method, params) { assert.equal(api, 'condenser_api'); assert.equal(method, 'get_active_votes'); assert.deepEqual(params, ['fartman69', 'beta-post']); return [{ voter: 'fartman69', rshares: '0', percent: 100 }, { voter: 'etblink', rshares: '0', percent: 100 }, { voter: 'downvoter', rshares: '0', percent: -100 }]; } },
    async getTransaction() { return null; },
    async getPostWithComments() { return structuredClone(baseDiscussion); },
    async observeSocialOperation() { return false; },
  };
  applyReadConsistencyHardening(hiveReads);
  ...
  hiveReads.rpcPool.call = async () => { throw new Error('temporary direct vote read failure'); };
