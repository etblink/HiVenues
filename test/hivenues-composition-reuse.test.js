...
function build(raw) {
  const result = buildHiVenuesHostFromInput(raw, { randomUUID: () => '22222222-2222-4222-8222-222222222222' });
...
test('composition families remain structural across physical, online, and hybrid fresh hosts after explicit Release', async (t) => {
  ...
  const store = new ProvisioningFileHiVenuesStore({ statePath: path.join(directory, 'state.json') });
  ...
  for (const { graph, expectedPresence } of cases) {
    ...
    const activityPage = await request(app).get(`/hivenues/${slug}/activities/${activity.slug}`).expect(200);
