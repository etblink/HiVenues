...
const { compileIntent } = require('./hivenues-h1-spike');

const CLIENT_PATH = path.join(__dirname, 'hivenues-h1b-client.js');
...
function seedGraph() {
  ...
  graph.kind = 'hivenues-h1b-host-graph';
  ...
  graph.media = [{ id: 'media-hero-001', kind: 'image', src: '/hivenues/media.svg', alt: 'Synthetic stage-light study', focal: { x: 50, y: 45 } }];
...
function createHiVenuesH1BSpike() {
  const state = createState();
  const app = express();
  app.use(express.urlencoded({ extended: false }));

  app.get('/hivenues/htmx.js', (_req, res) => res.type('application/javascript').send(fs.readFileSync(require.resolve('htmx.org/dist/htmx.min.js'), 'utf8')));
  app.get('/hivenues/h1b-client.js', (_req, res) => res.type('application/javascript').send(fs.readFileSync(CLIENT_PATH, 'utf8')));
  app.get('/hivenues/media.svg', (_req, res) => res.type('image/svg+xml').send('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 700"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#120f17"/><stop offset=".5" stop-color="#7c2d12"/><stop offset="1" stop-color="#f59e0b"/></linearGradient></defs><rect width="1200" height="700" fill="url(#g)"/><circle cx="790" cy="240" r="150" fill="#fff" opacity=".12"/><path d="M0 580 Q400 420 1200 600 V700 H0Z" fill="#000" opacity=".55"/></svg>'));
...
module.exports = { seedGraph, createState, createHiVenuesH1BSpike, renderCanvas, renderStudio };
