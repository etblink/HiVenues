...
function createHiVenuesH1Transport(options = {}) {
  const fixture = createHiVenuesH1Spike(options);
  const app = express();
  ...
  app.get('/hivenues/studio-island.js', (_request, response) => {
  ...
  app.use((request, response, next) => {
    ...
    response.send = (body) => {
      ...
      if (typeof output === 'string' && output.includes('</head>') && !output.includes('/hivenues/studio-island.js')) {
        output = output.replace('</head>', '<script src="/hivenues/studio-island.js" defer></script></head>');
      }
      return send(output);
    };
    return next();
