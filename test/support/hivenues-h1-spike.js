...
function renderDocument(graph, { title, canonicalPath, main, activity = null, draft = false }) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    ...
    <script src="/hivenues/htmx.js" defer></script>
...
function renderStudio(snapshot, activityId = 'activity-friday-001') {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>HiVenues Studio · ${escapeHtml(snapshot.draft.identity.displayName)}</title><script src="/hivenues/htmx.js" defer></script><style>${BASE_STYLES}${STUDIO_STYLES}</style></head>
...
function icsFor(graph, activity) {
  ...
  return ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//HiVenues HiVenues H1 Spike//EN', 'BEGIN:VEVENT', `UID:${activity.id}@hivenues.local`, `DTSTART:${stamp(activity.temporal.startAt)}`, `DTEND:${stamp(activity.temporal.endAt)}`, `SUMMARY:${activity.title.replaceAll(',', '\\,')}`, `LOCATION:${graph.facts.presence.address.replaceAll(',', '\\,')}`, 'END:VEVENT', 'END:VCALENDAR', ''].join('\r\n');
}

function createHiVenuesH1Spike(options = {}) {
  const source = compileIntent(options.intent);
  const state = createState(source);
  const app = express();
  app.use(express.urlencoded({ extended: false }));

  app.get('/hivenues/htmx.js', (_request, response) => {
