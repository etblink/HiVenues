...
test('Astra beta remediation: existing Activity can be rescheduled without identity change and stays draft-only until Release', async (t) => {
  ...
  const schedulePage = await request(app)
    .get(`/candidate-c/studio/${slug}/activity/${activity.id}/schedule`)
    .expect(200);
  ...
  await request(app)
    .post(`/candidate-c/studio/${slug}/activity/${activity.id}/schedule`)
    .type('form')
    .send({
      ...tokens(store, slug),
  ...
  await request(app)
    .post(`/candidate-c/studio/${slug}/release`)
    .type('form')
  ...
  const ics = await request(app)
    .get(`/candidate-c/${slug}/activities/${activity.slug}/calendar.ics`)
    .expect(200);
...
test('Astra beta remediation: Quick Connect accepts the same phone contact model as deep maintenance', async (t) => {
  ...
  const inspector = await request(app)
    .get(`/candidate-c/studio/${slug}/inspect?resource=connect`)
    .expect(200);
