...
test('draft preview activity navigation, calendar, and back-link remain on the working snapshot', async () => {
  ...
  const previewHome = await request(app).get(`/hivenues/studio/${slug}/preview`).expect(200);
  assert.match(previewHome.text, /Friday Night Assembly — Working Cut/);
  assert.match(previewHome.text, new RegExp(`/hivenues/studio/${slug}/preview/activities/${activitySlug}`));
  assert.doesNotMatch(previewHome.text, new RegExp(`href="/hivenues/${slug}/activities/${activitySlug}"`));

  const previewDetail = await request(app)
    .get(`/hivenues/studio/${slug}/preview/activities/${activitySlug}`)
    .expect(200);
  assert.match(previewDetail.text, /Friday Night Assembly — Working Cut/);
  assert.match(previewDetail.text, /Draft-only room notes/);
  assert.match(previewDetail.text, new RegExp(`href="/hivenues/studio/${slug}/preview"`));
  assert.match(previewDetail.text, /Draft preview/);
  assert.doesNotMatch(previewDetail.text, new RegExp(`action="/hivenues/${slug}/activities/${activitySlug}/rsvp"`));

  const previewCalendar = await request(app)
    .get(`/hivenues/studio/${slug}/preview/activities/${activitySlug}/calendar.ics`)
  ...
  assert.match(previewCalendar.text, new RegExp(`URL:/hivenues/studio/${slug}/preview/activities/${activitySlug}`));
  ...
  const publicDetail = await request(app).get(`/hivenues/${slug}/activities/${activitySlug}`).expect(200);
  ...
  const publicCalendar = await request(app)
    .get(`/hivenues/${slug}/activities/${activitySlug}/calendar.ics`)
    .expect(200);
...
test('all admitted public Directions keep preview Activity links inside preview context', async () => {
  ...
  for (const [slug, activitySlug] of cases) {
    ...
    assert.match(preview.text, new RegExp(`/hivenues/studio/${slug}/preview/activities/${activitySlug}`));
