...
<main class="shell" ${presentation ? 'data-editable-canvas-surface' : 'data-read-only-canvas-surface'} ${attrs}>
  <header class="topbar"><div><p class="eyebrow">HiVenues · Venue Studio</p><h1>${escapeHtml(sourceInput.venueContext.displayName)}</h1></div><div class="top-actions"><span class="badge">${presentation ? 'Canvas preview' : 'Read-only Canvas'}</span><a class="back" href="${escapeHtml(editorPath)}">Back to form editor</a></div></header>
  ...
  <div class="workspace">
    ...
    <aside class="panel inspector" data-inspector ${attrs} aria-labelledby="inspector-heading"><header class="panel-head"><h2 id="inspector-heading" tabindex="-1">Inspector</h2><span class="badge">${presentation ? 'Canvas preview' : 'Read-only'}</span></header><div class="inspector-context" data-inspector-block-id="${escapeHtml(selection.blockId)}"><h3>${escapeHtml(selectedLabel)}</h3><p>${presentation ? 'Choose an editable text field or supported item action to preview a change.' : 'Choose a field to inspect its context.'}</p></div>${presentation?.inspector || ''}${fieldControls}
